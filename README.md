...
* Fixed the error, that only the first of several holes in the file is drilled with the correct depthm whereas all following holes are only spot drilled. Now alle holed are drilled corretly with the given depth.
* When generating g-code for only one layer (TOP or BOOTOM), the files for TEXT and MILL are generated for the opposite layer as well.
